...
class ProxySandbox {
    constructor(name, globalContext, options = {}) {
        this.name = name;
        ...
        const proxy = new Proxy(fakeGlobal, {
            set: (target, prop, value) => {
                if (this.sandboxRunning) {
                    if (typeof prop === 'string' && globalVariableWhiteList.indexOf(prop) !== -1) {
            ...
            get: (target, prop) => {
                ...
                if (prop === 'string' && globalVariableWhiteList.indexOf(prop) !== -1) {
